...
- Privacy: Nuxt telemetry disabled by default via NUXT_TELEMETRY_DISABLED=1.
- Directory Management: Nuxt build creates both .nuxt and .output. The .nuxt directory contains build artifacts for mounting, while .output contains the server runtime and should remain in place for the start command.

### Build Process
1. Install dependencies and build application
2. Move build output to temporary location (.nuxt-built)

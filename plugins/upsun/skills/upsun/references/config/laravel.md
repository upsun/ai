...
### PHP Build & Runtime Notes

- Always set `build.flavor: none` for explicit Composer control and production flags.
- Use the build hook to write a `.environment` file for secrets derived at build time (do not hard-code secrets in `variables.env`).
- Adjust `runtime.extensions` for additional PHP extensions your app requires.


Note: Verify runtime and service versions against the canonical registry before committing, at https://meta.upsun.com/images — registry versions are deprecated/retired over time.

...
**Configuration Notes**:
- **Runtime Version**: Choose appropriate Go version from Registry; this reference uses golang:1.25
- **Build Command**: Adapt binary name and package path; use a Makefile if available (`make build`)
...
If using MariaDB or MySQL (for example with `go-sql-driver/mysql`), set a DSN in the build hook:
...
If extra command(s) should be long-running processes and share the same configuration as the application, run them as Upsun worker containers. Example:

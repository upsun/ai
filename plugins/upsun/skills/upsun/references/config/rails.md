...
- Ruby Version: Use the exact Ruby runtime supported by Upsun (example above uses ruby:3.4).
...
- Database URL: Rails can auto-discover database configuration via the DATABASE_URL environment variable.
- File Management: Persistent mounts for storage and logs; tmp mount for temporary files.
- Port Handling: The Rails server uses the $PORT environment variable provided by Upsun.
...
- For Rails versions before Rails 8, on the build hook replace the DATABASE_URL line with:
...
- URL scheme: sqlite3://
- Requires persistent storage mount for the database file to persist data

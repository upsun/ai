...
To modify the application to use MariaDB instead of PostgreSQL, change the type of the related database service to MariaDB and change the app's extensions to use `pdo_mysql` (instead of `pdo_pgsql`). Example service type to use from the registry: `mariadb:11.8`.
...
Note: verify image and service versions against the canonical registry before committing, see: https://meta.upsun.com/images

...
To modify the application to use MariaDB instead of PostgreSQL, change the type of the related database service to MariaDB (e.g. to `mariadb:11.8` instead of `postgresql:18`) and change the app's extensions to use `pdo_mysql` (instead of `pdo_pgsql`).
...
When the `symfony/messenger` Composer package is installed, you must add a worker in the `workers` section that consumes messages:

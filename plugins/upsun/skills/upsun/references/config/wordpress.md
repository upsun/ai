...
applications:
  app:
    ...
    dependencies:
      php:
        wp-cli/wp-cli-bundle: '^2' # Install the WordPress CLI (required for the deploy hook and crons to work).
    ...
    # These files (if they exist) may need to be modified to adapt the application to Upsun configuration.
    file_modifications:
      - wp/wp-config.php
      - .env
...
Note: Images and service versions in this file use the registry snapshot available at https://meta.upsun.com/images. Always verify and prefer supported versions from that canonical registry before committing, since versions are deprecated or retired over time.

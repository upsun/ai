...
applications:
  app:
    ...
    stack:
      packages:
        - hugo  # Uses stable version from nixpkgs (via the composable image)
...
If you need a specific Hugo version not yet available in Nix packages, use the Upsun-provided script to download the Hugo binary. If your project lives in a subdirectory, add a `source.root: <path>` entry under the application.
...
Note: verify any runtime or service image versions you reference against the canonical registry before committing, at https://meta.upsun.com/images — versions are deprecated or retired over time.

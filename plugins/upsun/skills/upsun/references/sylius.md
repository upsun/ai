...
applications:
  sylius:
    ...
    dependencies:
      nodejs:
        n: "*"
        yarn: "*" # If yarn is used, install it here.
...
PHP Version: Sylius requires PHP 8.1 or higher. Use the latest stable PHP version from the registry (example above uses php:8.4).
...
- Service type: mariadb:11.4 or mysql:8.0
...
Cache Storage:
- var/cache: Symfony cache files (source set to instance in example for performance)

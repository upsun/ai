...
- **Never put secrets in `variables.env`** — use a build hook to create a `.environment` file or other secure secret mechanism.
- **Use `PLATFORM_PROJECT_ENTROPY` for secrets**, for example to derive Symfony's `APP_SECRET` at build time.

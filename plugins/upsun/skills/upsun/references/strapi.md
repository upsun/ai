...
- Database Client: Set via DATABASE_CLIENT and by the relationship service type
- Admin Panel: Accessible through Strapi's built-in interface
- API Access: All routes pass through to the Strapi application

Database Setup:
- Strapi automatically configures database connection from DATABASE_* variables Upsun provides for a relationship named `database`
- No manual environment variable configuration required for typical setups
